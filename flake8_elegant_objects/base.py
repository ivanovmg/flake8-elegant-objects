...
class ElegantObjectsAnalysis(ABC):
    """Base class for Elegant Objects violation analysis."""
    ...
    def analyze(self, node: ast.AST) -> list[Violation]:
        """Analyze node for violations and return list of detected violations."""
        
    def _add_violation(self, node: ast.AST, message: str) -> None:
        """Add a violation if node has location information."""
        if hasattr(node, "lineno") and hasattr(node, "col_offset"):
            violation = Violation(node.lineno, node.col_offset, message)
